...
public class UpdateKilometers {
	...
	protected void createContents() {
		...
		lblFuelUsageWarning.setForeground(SWTResourceManager.getColor(SWT.COLOR_RED));
	...
	private void SetFields() {
		currVehicle.updateKm(new Integer(textKms.getText()), new Double(textFuelUsage.getText())); 
	}
	...
	private boolean checkKms()
	{
		...
		if(!isValid){
			...
			lblKmsWarning.setText("Updated kilometers must be more than previous kilometers");
		}
		else{
	...
	private boolean checkFuelUsage()
	{
		...
		isValid = input.matches("[0-9]+") && input.matches("[0-9]*");
		if(!isValid){
			lblFuelUsageWarning.setText("Fuel usage serviced can only be expressed by numbers");
		}
		else{
