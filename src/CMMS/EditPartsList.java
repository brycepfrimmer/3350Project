...
public class EditPartsList {
	...
	protected void createContents() {
		...
		btnRemove.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (selected == 0) {
					...
					MessageBox mb = new MessageBox(shlEditPartsList, SWT.ICON_ERROR | SWT.OK );
					...
					MessageBox mb = new MessageBox(shlEditPartsList, SWT.ICON_WARNING | SWT.YES | SWT.NO);
					...
					if (response == SWT.YES) {
						...
						for (int i = 0; i < selected; i++)
							list.removePart(partsListTable.getItem(selections[i]).getText(0));
					...
					if (response == SWT.YES) {
						...
						list.removePart(partsListTable.getItem(partsListTable.getSelectionIndex()).getText());
						// Update list with the new Vehicles
						updateList();
					}
				}
		...
		partsListTable = new Table(shlEditPartsList, SWT.BORDER | SWT.FULL_SELECTION);
